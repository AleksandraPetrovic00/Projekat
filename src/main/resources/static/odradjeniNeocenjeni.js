$(document).ready(function() {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost:8090/api/ocene/odradjeniNeocenjeni/"+id,
        dataType: "json",
        success: function(response) {
            ...
            for(let trening of response){
                ...
                row+="<td><button class='oceni' data-id='"+trening.id+"' onClick=window.location.href='oceni.html'>OCENI</button></td>";
                row+="</tr>";
    ...
    $(document).on('click', '.oceni', function(){
        let oceniTerminId = this.dataset.id;
        console.log(oceniTerminId);
        localStorage.setItem("oceniTerminId", oceniTerminId);
    })
